...
class Config:
    def __init__(
            ...
            encoder_save_path: str = 'model_weight/pretrain_MAE_encoder_IQ_20shot.pth',
            decoder_save_path: str = 'model_weight/pretrain_MAE_decoder_IQ_20shot.pth',
            device_num: int = 0,
            k_shot: int = 20,
    ):
...
def main():
    ...
    writer = SummaryWriter("logs_FS_AMAE_IQ")
    dev_range = np.arange(0, 30, dtype=int)
    pkt_range = np.arange(0, 500, dtype=int)
    RANDOM_SEED = 300  # any random number
    set_seed(RANDOM_SEED)

    X_train, X_val, Y_train, Y_val = get_num_class_FStraindata(conf.n_classes, conf.k_shot, conf.train_file_path, dev_range, pkt_range)
