...
from get_LoRa_IQ_dataset import *
...
def main():
    ...
    X_test, Y_test = get_num_class_testdata(num, test_file_path, dev_range, pkt_range)
    test_dataset = TensorDataset(torch.Tensor(X_test), torch.Tensor(Y_test))
    test_dataloader = DataLoader(test_dataset, batch_size=64, shuffle=True)
    # model = torch.load('model_weight/pretrain_MAE_encoder_IQ.pth')
    model = torch.load('model_weight/pretrain_MAE_encoder_IQ_20shot.pth')
    ...
    scatter(eval_tsne_embeds, real_target.astype('int64'), "20shot_AMAE_IQ", 30)
