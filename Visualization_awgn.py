...
def main():
    ...
    test_file_path = '/data/yaozs/dataset/LoRa_dataset/Test/dataset_seen_devices.h5'
    num = [0,30]
    snr = 0

    X_test, Y_test = get_num_class_awgn_testdata(num, snr, test_file_path, dev_range, pkt_range)
    test_dataset = TensorDataset(torch.Tensor(X_test), torch.Tensor(Y_test))
    test_dataloader = DataLoader(test_dataset, batch_size=64, shuffle=True)
    # model = torch.load('model_weight/pretrain_MAE_encoder_IQ.pth')
    model = torch.load('model_weight/pretrain_MAE_encoder_IQ_20dB.pth')
    ...
    # scatter(eval_tsne_embeds, real_target.astype('int64'), "AMAE_IQ", 30)
    scatter(eval_tsne_embeds, real_target.astype('int64'), "AMAE_IQ_10dB", 30)

    km = KMeans(n_clusters=30, n_init=30)
    km.fit(eval_tsne_embeds)
    cluster_target = km.predict(eval_tsne_embeds)
    cluster_target = np.expand_dims(cluster_target, axis=1)

    cluster_target = np.squeeze(cluster_target, axis=1)
    ...
    # get_accuracy_score(cluster_target, real_target)
    # get_classification_report(X_test_embedding_feature_map, target.astype('int64'))
