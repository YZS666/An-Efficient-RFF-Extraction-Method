...
def train_and_validation(encoder,
    ...
    df.to_excel(f"test_result/AMAE_S_SC_MSE.xlsx")
    df = pd.DataFrame(loss_mse_all)
    df.to_excel(f"test_result/AMAE_S_MSE.xlsx")
    df = pd.DataFrame(loss_sc_mse_all)
    df.to_excel(f"test_result/AMAE_S_SC.xlsx")
...
def main():
    ...
    X_train, X_val, Y_train, Y_val = get_num_class_traindata(conf.n_classes, conf.train_file_path, dev_range, pkt_range)
